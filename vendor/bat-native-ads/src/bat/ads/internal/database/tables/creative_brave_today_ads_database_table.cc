...
void CreativeBraveTodayAds::Delete(
    ResultCallback callback) {
  DBTransactionPtr transaction = DBTransaction::New();
...
void CreativeBraveTodayAds::GetAll(
    GetCreativeBraveTodayAdsCallback callback) {
  const std::string query = base::StringPrintf(
      "SELECT "
          "can.creative_instance_id, "
  ...
  AdsClientHelper::Get()->RunDBTransaction(std::move(transaction),
      std::bind(&CreativeBraveTodayAds::OnGetAll, this, std::placeholders::_1,
          callback));
}

void CreativeBraveTodayAds::set_batch_size(
...
void CreativeBraveTodayAds::Migrate(
...
int CreativeBraveTodayAds::BindParameters(
  ...
  for (const auto& creative_brave_today_ad : creative_brave_today_ads) {
    ...
    BindString(command, index++, creative_brave_today_ad.company_name);
    BindString(command, index++, creative_brave_today_ad.alt);
...
CreativeBraveTodayAdInfo CreativeBraveTodayAds::GetFromRecord(
  ...
  creative_brave_today_ad.company_name = ColumnString(record, 14);
  creative_brave_today_ad.alt = ColumnString(record, 15);
...
void CreativeBraveTodayAds::CreateTableV8(
    DBTransaction* transaction) {
  DCHECK(transaction);
...
void CreativeBraveTodayAds::MigrateToV8(
    DBTransaction* transaction) {
  DCHECK(transaction);

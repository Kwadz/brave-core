...
BraveTodayAdsPerDayFrequencyCap::BraveTodayAdsPerDayFrequencyCap(
...
bool BraveTodayAdsPerDayFrequencyCap::DoesRespectCap(
  ...
  return DoesHistoryRespectCapForRollingTimeConstraint(
      history, time_constraint, kBraveTodayAdsPerDayFrequencyCap);
}

AdEventList BraveTodayAdsPerDayFrequencyCap::FilterAdEvents(
    const AdEventList& ad_events) const {
  AdEventList filtered_ad_events = ad_events;

  const auto iter = std::remove_if(filtered_ad_events.begin(),
      filtered_ad_events.end(), [](const AdEventInfo& ad_event) {
    return ad_event.type != AdType::kBraveTodayAd ||

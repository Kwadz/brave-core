...
BraveTodayAdsPerHourFrequencyCap::BraveTodayAdsPerHourFrequencyCap(
    const AdEventList& ad_events)
    : ad_events_(ad_events) {
}

BraveTodayAdsPerHourFrequencyCap::~BraveTodayAdsPerHourFrequencyCap() = default;

bool BraveTodayAdsPerHourFrequencyCap::ShouldAllow() {
  const AdEventList filtered_ad_events = FilterAdEvents(ad_events_);
  if (!DoesRespectCap(filtered_ad_events)) {
    last_message_ = "You have exceeded the allowed Brave Today ads per hour";
    return false;
  }
...
std::string BraveTodayAdsPerHourFrequencyCap::get_last_message() const {
  return last_message_;
}

bool BraveTodayAdsPerHourFrequencyCap::DoesRespectCap(
    const AdEventList& ad_events) {
  ...
  return DoesHistoryRespectCapForRollingTimeConstraint(
      history, time_constraint, kBraveTodayAdsPerHourFrequencyCap);
}

AdEventList BraveTodayAdsPerHourFrequencyCap::FilterAdEvents(

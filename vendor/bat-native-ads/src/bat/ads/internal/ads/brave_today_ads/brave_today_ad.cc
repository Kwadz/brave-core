...
BraveTodayAdInfo CreateBraveTodayAd(
  ...
  brave_today_ad.company_name = ad.company_name;
  brave_today_ad.alt = ad.alt;
...
void BraveTodayAd::AddObserver(
    BraveTodayAdObserver* observer) {
  DCHECK(observer);
  observers_.AddObserver(observer);
}

void BraveTodayAd::RemoveObserver(
    BraveTodayAdObserver* observer) {
  DCHECK(observer);
  observers_.RemoveObserver(observer);
}

void BraveTodayAd::FireEvent(
    const std::string& uuid,
    const std::string& creative_instance_id,
    const BraveTodayAdEventType event_type) {
  if (uuid.empty() || creative_instance_id.empty()) {
    BLOG(1, "Failed to fire Brave Today ad event for uuid " << uuid
...
bool BraveTodayAd::ShouldFireEvent(
    const BraveTodayAdInfo& ad,
...
void BraveTodayAd::NotifyBraveTodayAdViewed(
    const BraveTodayAdInfo& ad) {
...
void BraveTodayAd::NotifyBraveTodayAdClicked(
    const BraveTodayAdInfo& ad) {
  for (BraveTodayAdObserver& observer : observers_) {

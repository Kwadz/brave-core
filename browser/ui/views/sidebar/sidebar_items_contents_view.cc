...
void SidebarItemsContentsView::SetDefaultImageAt(
  ...
  float scale_factor = GetWidget()->GetCompositor()->device_scale_factor();
  gfx::Canvas canvas(kIconSize, scale_factor, false);

  // TODO(simonhong): Ask this design to UX team.
  // Use bigger font size for text rendered favicon.
  ...
  SetImageForItem(
      item,
      gfx::ImageSkia(gfx::ImageSkiaRep(canvas.GetBitmap(), scale_factor)));
}

...
void SidebarModel::OnURLVisited(history::HistoryService* history_service,
  ...
  for (int i = 0; i < item_count; ++i) {
    if (items[i].url == row.url() && data_[i]->need_favicon_update()) {
      // TODO(simonhong): Improve this logic.
      // Fetch happens after second loading because this notification arrived
      // before favicon is fetched.
      return FetchFavicon(items[i]);
    }
  }
}
...
void SidebarModel::FetchFavicon(int index) {
  if (data_[index]->need_favicon_update())
    FetchFavicon(GetAllSidebarItems()[index]);
}
...
void SidebarModel::OnGetFaviconImageFromNetwork(
  ...
  // TODO(simonhong): We should set default image or lettering if we can't get
  // favicon from website.
}
